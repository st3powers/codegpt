...
pres.z <- cbind(pres, raster::extract(env, pres))
pres.z$biome <- factor(pres.z$biome)
bg <- as.data.frame(dismo::randomPoints(env, 1000))
names(bg) <- names(pres)
...
e.maxent.jar <- ENMevaluate(pres, env, bg, 
...
pred.L2 <- enm.maxent.jar@predict(mods.maxent.jar$fc.L_rm.2, env, os)
raster::plot(pred.L2)
