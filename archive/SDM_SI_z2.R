...
buffer_distance_m <- 1e6
max_cor <- 0.7
regularization <- c(0, 1, 5, 10, 100)
...
env <- raster::stack(list.files(path=paste(system.file(package="dismo"), "/ex", sep=""), 
  ...
  proj4string(pres_sp) <- CRS("epsg:4326")
  
  # Step 2: Remove duplicated pres in the cells of env
#  env_no_dup <- raster::raster::aggregate(env, fact = round(res(env) / res(pres_sp)))   
#  env_no_dup <- raster::aggregate(env, fact = round(raster::res(env),digits=0)) # / raster::res(pres_sp)))  
#  env_no_dup <- raster::aggregate(env, fact = 1) 
...
#  env_moll <- projectRaster(env_no_dup, crs = sp::CRS("+proj=moll +lon_0=0 +x_0=0 +y_0=0"))   
  env_moll <- projectRaster(env, crs = sp::CRS("+proj=moll +lon_0=0 +x_0=0 +y_0=0"))   
...
#  env3 <- env2[, -to_remove]      
  ...
  bg <- as.data.frame(dismo::randomPoints(env3, 1000))
  names(bg) <- names(pres)
